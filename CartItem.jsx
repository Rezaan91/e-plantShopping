      ...
      {items.length === 0 ? (
        ...
        <div style={{display:'grid', gridTemplateColumns:'2fr 1fr', gap:'1.2rem'}}>
          <div style={{display:'grid', gap:'0.8rem'}}>
            {items.map(it => (
              <article key={it.id} style={{display:'grid', gridTemplateColumns:'96px 1fr auto', gap:'1rem', alignItems:'center', border:'1px solid #e5e7eb', borderRadius:12, padding:'0.6rem 0.8rem', background:'#fff'}}>
                ...
                <div style={{display:'flex', alignItems:'center', gap:8}}>
                  ...
                  <button onClick={()=>dispatch(removeItem(it.id))} style={{background:'transparent', border:'1px solid #ef4444', color:'#ef4444', padding:'.4rem .6rem', borderRadius:8, cursor:'pointer'}}>Remove</button>
                </div>
              </article>
            ))}
          </div>
          <aside style={{border:'1px solid #e5e7eb', borderRadius:12, padding:'1rem', height:'fit-content', background:'#fff'}}>
            ...
            <button disabled={items.length===0} style={{width:'100%', background:'#2e7d32', color:'#fff', border:'none', padding:'0.8rem 1rem', borderRadius:8, cursor:'pointer'}}>Checkout</button>
          </aside>
        </div>
      )}
    </section>
